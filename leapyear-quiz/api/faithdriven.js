/**
 * LeapYear — Faith Driven Conference Lead Capture
 * POST /api/faithdriven
 *
 * Captures email from the Faith Driven Conference intermediary page,
 * writes to Airtable People table with Faith Driven lead source attribution.
 */
...
const FAITH_DRIVEN_LEAD_SOURCE_ID = process.env.FAITH_DRIVEN_LEAD_SOURCE_ID || 'recaX35raONL937Y3';
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (existing) {
      // Append Faith Driven lead source if not already present
      const updates = {};
      if (!existing.leadSources.includes(FAITH_DRIVEN_LEAD_SOURCE_ID)) {
      ...
      const fields = {
        ...
        [PPL.createLead]: true,
        [PPL.leadSource]: [FAITH_DRIVEN_LEAD_SOURCE_ID],
        [PPL.type]: ['Student'],
      };
      if (gradYear) {
